...
client.on("interactionCreate", async i => {
  ...
  if (i.isModalSubmit() && i.customId.startsWith("earn_modal_")) {
    ...
    const row = new ActionRowBuilder().addComponents(
      ...
      new ButtonBuilder().setCustomId("earn_reject").setLabel("Отклонить").setStyle(ButtonStyle.Danger)
    );
  ...
  if (i.isStringSelectMenu() && i.customId === "upgrade_select") {
    ...
    modal.addComponents(
      new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId("nick").setLabel("Ник + статик").setStyle(TextInputStyle.Short)),
      new ActionRowBuilder().addComponents(new TextInputBuilder().setCustomId("proof").setLabel("Ссылка/скрин").setStyle(TextInputStyle.Short))
    );
